"""Модуль-мост между HerdrControlCenter, Anthropic Claude Code и AionUi.

Обеспечивает автоматизацию всех шагов интеграции:
1. Валидацию и импорт OAuth2-токенов Claude Pro/Team (.credentials.json) в Windows и WSL2.
2. Проверку срока действия токенов (accessToken, refreshToken) и подписки.
3. Синхронизацию прокси (SOCKS5 :1015 / HTTP CONNECT :11015) и защиту Killswitch в settings.json.
4. Автоматическое обновление встроенных ресурсов AionUi (manifest.json, binary version 2.1.280+).
5. Регистрацию исполняемого файла claude в базе данных AionUi (agent_metadata).
6. Комплексную проверку здоровья и статуса подключения.
