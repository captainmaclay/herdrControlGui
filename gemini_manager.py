...
маршрутизируемых через независимые сетевые узлы, превзойти монолитные старшие модели
по точности, полноте рассуждений и стабильности инференса?».

Обеспечивает:
- Управление профилями рабочих узлов инференса в WSL2 (~/.gemini/profiles)
- Жесткую сетевую изоляцию: 1 рабочий узел = 1 отдельный локальный порт (1081, 1082, 1083...)
  для исключения взаимной интерференции очередей и чистоты замеров задержек (TTFT/TPS)
