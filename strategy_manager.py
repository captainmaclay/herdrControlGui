"""Модуль управления стратегией прокси и аналитикой совокупного времени соединений.

Функционал:
1. Хранение журнала сессий и времени подключений в strategy_history.json
2. Анализ тенденций использования прокси по СОВОКУПНОМУ ВРЕМЕНИ СОЕДИНЕНИЙ (вместо подсчета количества сессий)
3. Формирование рекомендаций:
   - 🟢 ОК — текущий SOCKS5 соответствует доминирующей стране по времени соединений
   - 🟡 Nice to change — рекомендуется сменить прокси на страну наибольшей длительности соединений
4. Отказоустойчивость к экспорту и импорту бэкапов:
   - Автоматическая валидация и исправление структуры записей при загрузке
   - Совместимость с архивными бэкапами (миграция старых записей без duration_seconds)
   - Атомарная запись через временный файл для защиты от повреждений
5. Расширенный лог: длительность каждого сеанса и суммарное время по уникальным IP
"""
